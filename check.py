...
* **Seasonality:** Time-based features suchs as `day_of_week_num` (0.014, 0.013) and `day_of_week_Saturday` (0.010) play a consistent role, confirming predictable fluctuations in demand based on the day of the week.
...
st.sidebar.download_button(
    label="⬇️ Download Executive Summary (Markdown)",
    data=executive_summary_content,
    file_name="FrischMarkt_Executive_Summary.md",
    mime="text/markdown"
)
...
with tab1:
    ...
    fig_trends.add_trace(go.Scatter(x=daily_loss['date'], y=daily_loss['total_loss_eur'], mode='lines', name='Total Daily Loss (€)', line=dict(color=PRIMARY_COLOR))) # Warm red
